...
public class WeatherDB {
    ...
    public static List<Province> loadProvinces(SQLiteDatabase db) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                province.setMProSort(cursor.getInt(cursor.getColumnIndex("ProSort")));
                province.setMProName(cursor.getString(cursor.getColumnIndex("ProName")));
                list.add(province);
            } while (cursor.moveToNext());
        }
        Util.INSTANCE.closeQuietly(cursor);
        return list;
    }
    ...
    public static List<City> loadCities(SQLiteDatabase db, int ProID) {
        ...
        if (cursor.moveToFirst()) {
            do {
                City city = new City();
                city.setMCityName(cursor.getString(cursor.getColumnIndex("CityName")));
                city.setMProID(ProID);
                city.setMCitySort(cursor.getInt(cursor.getColumnIndex("CitySort")));
                list.add(city);
            } while (cursor.moveToNext());
        }
        Util.INSTANCE.closeQuietly(cursor);
