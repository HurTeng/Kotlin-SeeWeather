...
object RxUtil {
    ...
    fun <T> activityLifecycle(activity: RxAppCompatActivity): ObservableTransformer<T, T> {
        return ObservableTransformer { observable -> observable.compose(activity.bindUntilEvent<T>(ActivityEvent.DESTROY)) }
    }

    fun <T> fragmentLifecycle(fragment: RxFragment): ObservableTransformer<T, T> {
        return ObservableTransformer { observable -> observable.compose(fragment.bindUntilEvent<T>(FragmentEvent.DESTROY)) }
    }

    fun <T> activityLifecycleF(activity: RxAppCompatActivity): FlowableTransformer<T, T> {
        return FlowableTransformer { flowable -> flowable.compose(activity.bindUntilEvent<T>(ActivityEvent.DESTROY)) }
    }

    fun <T> fragmentLifecycleF(fragment: RxFragment): FlowableTransformer<T, T> {
        return FlowableTransformer { flowable -> flowable.compose(fragment.bindUntilEvent<T>(FragmentEvent.DESTROY)) }
    }
